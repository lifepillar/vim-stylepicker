...
var BlankViewMemo = react.Memo(POOL)
# }}}
# TitleView {{{
...
var TitleViewMemo = react.Memo(POOL)
# }}}
# StepView {{{
...
var StepViewMemo = react.Memo(POOL)
# }}}
# ColorInfoView {{{
...
var ColorInfoViewMemo = react.Memo(POOL)
# }}}
# QuotationView {{{
...
var QuotationViewMemo = react.Memo(POOL)
# }}}
...
def DrawTextLine(text: string, props: list<RichText>)
...
def RgbPane(winID: number)
  ...
  react.CreateEffect(() => {
    var view = TitleView()
    setbufline(bufnr, 1, view[0].text)

    for prop in view[0].props
      prop_add(1, prop.col, {type: prop.type, length: prop.length, bufnr: bufnr})
    endfor
  })
enddef
# }}}
...
const HsbView = VStack(
  ...
  StepViewMemo.Get,
  BlankViewMemo.Get,
  ColorInfoViewMemo.Get,
  BlankViewMemo.Get,
  QuotationViewMemo.Get,
  BlankViewMemo.Get,
  RecentViewMemo.Get,
  BlankViewMemo.Get,
  FavoriteViewMemo.Get,
)
...
def InitEffects(winID: number, bufnr: number)
  ...
  react.CreateMemo(BlankViewMemo,     BlankView)
  react.CreateMemo(TitleViewMemo,     TitleView)
  react.CreateMemo(StepViewMemo,      StepView)
  react.CreateMemo(ColorInfoViewMemo, ColorInfoView)
  react.CreateMemo(QuotationViewMemo, QuotationView)
  react.CreateMemo(RecentViewMemo,    RecentView)
  react.CreateMemo(FavoriteViewMemo,  FavoriteView)
