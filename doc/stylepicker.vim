*stylepicker.txt*	For Vim version 9.1	Last change: 2025 Apr 27
...
|g:stylepicker_ascii| to |true|. If the plugin is loaded, just type this in the
command line:
>
	:let stylepicker#Set('ascii', 1)
<
The change will take effect immediately. See |stylepicker-customization| for
...
>
	◀ Rgb Hsb Gray ?Help ▶
<
ll color chooser panes have a header, which shows some properties of the
...
those attributes, the corresponding letter is highlighted. You may type those
letters to toggle the corresponding attribute. You may also click on the
letters to do the same.
...
a `❯❯` marker (or `>>` in ASCII mode) on the side. For instance:
>
	❯❯ R 149 ███████████████████▎
	   G 199 █████████████████████████▊
	   B 111 ██████████████▍
...
If your terminal does not support true colors, the left swath will be of
course an approximation of the true color value, so it will not be very
useful.
...
a recent color. Each of those keys will prompt you pick a color by typing
...
>
	...
	❯❯ ███ ███ ███ ███ ███ ███ ███ ███ ███ ███
	   ...
	   ███ ███ ███ ███
<
When you press |A|, the current color is added to the Favorite Colors palette.
...
==============================================================================
The style picker popup				*stylepicker-customization*
...
with eight elements, defining the character to use for the
top/right/bottom/left border, followed by the characters to use for the
...
working directory#ffffff. Use an absolute path to avoid ambiguities.
>
	g:stylepicker_favoritepath = ''
<
						*'g:stylepicker_highlight'*
Highlight group name to use for the stylepicker's text. See "highlight" in
...
Number of recent colors to remember. Do not change interactively!

						*'g:stylepicker_quotes'*
List of quotations to show in the style picker. Every time the style picker is
reopened, a quotation is randomly chosen from this list. Each quotation should
be at most 42 characters long.
